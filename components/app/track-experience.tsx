...
function TrackVideoSection({ locale, trackId, videoUrl, videoFileName, admin, title }: { locale: Locale; trackId: string; videoUrl?: string | null; videoFileName?: string; admin: boolean; title?: Copy }) {
  ...
  return <section className="relative rounded-[16px] border-2 border-ink bg-paper-strong p-3 shadow-[6px_6px_0_var(--shadow-hard-10)]">
    <div className="relative overflow-hidden rounded-[8px]">
      ...
      {(shownTitle || admin) ? <div className="pointer-events-none absolute inset-x-4 bottom-4 rounded-[16px] bg-scrim/88 px-5 py-3 text-white backdrop-blur"><div className="pointer-events-auto type-h2"><InlineContentText entityId={trackId} kind="track" field="pageCopy_introVideoTitle" locale={locale} sr={title?.sr ?? ""} en={title?.en ?? ""} admin={admin}>{shownTitle || "Dodaj naslov videa"}</InlineContentText></div></div> : null}
    </div>
...
export function TrackExperience({ data, locale, admin = false, inlineLocale = locale, profileName = "student" }: { data: TrackExperienceData; locale: Locale; admin?: boolean; inlineLocale?: Locale; profileName?: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-6">
      <section className="overflow-hidden rounded-[16px] border-2 border-ink bg-paper-strong shadow-[8px_8px_0_var(--shadow-hard-13)]">
        <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="p-6 sm:p-8">
            <div className="flex flex-wrap items-center gap-2"><span className="rounded-full border-2 border-ink bg-yellow px-4 py-1.5 type-eyebrow">{data.track.status === "published" ? "Objavljen smer" : "Nacrt"}</span></div>
            <p className="mt-7 type-eyebrow text-muted">{locale === "sr" ? `Zdravo, ${profileName}` : `Hi, ${profileName}`}</p>
            <h1 className="mt-3 font-display type-display text-ink"><InlineContentText entityId={trackId} kind="track" field="title" locale={inlineLocale} sr={data.track.titleSr} en={data.track.titleEn} admin={admin && Boolean(trackId)}>{local(locale, data.track.titleSr, data.track.titleEn)}</InlineContentText></h1>
            <p className="mt-4 type-body type-measure font-black text-muted"><InlineContentText entityId={trackId} kind="track" field="subtitle" locale={inlineLocale} sr={data.track.subtitleSr ?? ""} en={data.track.subtitleEn ?? ""} admin={admin && Boolean(trackId)}>{local(locale, data.track.subtitleSr, data.track.subtitleEn)}</InlineContentText></p>
            <InlineRichText kind="track" entityId={trackId} field="description" locale={inlineLocale} richSr={data.track.descriptionRichSr} richEn={data.track.descriptionRichEn} sr={data.track.descriptionSr ?? ""} en={data.track.descriptionEn ?? ""} admin={admin && Boolean(trackId)} className="mt-6 type-reading type-measure font-semibold text-muted" />
          ...
          <div className="flex min-h-72 flex-col justify-between bg-ink p-6 text-paper-strong"><p className="type-eyebrow text-paper-strong/60">Pregled smera</p><p className="font-display type-display">{data.courses.length}</p><p className="text-sm font-black">{data.courses.length === 1 ? "kurs" : "kurseva"}</p></div>
        </div>
      </section>
      ...
      {data.featuredThreads?.length ? <section><h2 className="font-display type-display-sm text-ink">Izdvojeno iz zajednice</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{data.featuredThreads.map((thread) => <Link key={thread._id} href={withLocale(locale, `/app/community/${thread._id}`)} className="rounded-[16px] border-2 border-ink bg-paper-strong p-6 shadow-[4px_4px_0_var(--shadow-hard-10)]"><p className="font-black">{thread.title}</p><p className="mt-2 line-clamp-2 text-sm font-semibold text-muted">{thread.body}</p></Link>)}</div></section> : null}
      <section id="kursevi" className="scroll-mt-6"><div><p className="type-eyebrow text-muted"><InlineContentText entityId={trackId} kind="track" field="pageCopy_sectionEyebrow" locale={inlineLocale} sr={sectionEyebrow?.sr ?? "Nastavni sadržaj"} en={sectionEyebrow?.en ?? "Learning content"} admin={admin && Boolean(trackId)}>{local(locale, sectionEyebrow?.sr ?? "Nastavni sadržaj", sectionEyebrow?.en ?? "Learning content")}</InlineContentText></p><h2 className="mt-2 font-display type-display-sm text-ink"><InlineContentText entityId={trackId} kind="track" field="pageCopy_sectionTitle" locale={inlineLocale} sr={sectionTitle?.sr ?? "Kursevi ovog smera"} en={sectionTitle?.en ?? "Courses in this track"} admin={admin && Boolean(trackId)}>{local(locale, sectionTitle?.sr ?? "Kursevi ovog smera", sectionTitle?.en ?? "Courses in this track")}</InlineContentText></h2></div><div className="mt-5 grid gap-6 md:grid-cols-2">{data.courses.map((course) => <DashboardCourseCard key={course._id} locale={locale} course={asDashboardCourse(course)} isAdmin={admin} />)}</div></section>
    </div>
  );
}
