...
function TrackVideoSection({ locale, trackId, videoUrl, videoFileName, admin, title }: { locale: Locale; trackId: string; videoUrl?: string | null; videoFileName?: string; admin: boolean; title?: Copy }) {
  ...
  return <section className="relative rounded-[16px] border-2 border-ink bg-white p-3 shadow-[6px_6px_0_rgba(14,49,88,0.1)]">
    ...
    {dragging ? <div className="fixed inset-0 z-[200] grid place-items-center bg-ink/88 p-6 text-center text-white backdrop-blur"><div className="rounded-[16px] border-2 border-white bg-ink p-8 shadow-[8px_8px_0_#f4be30]"><UploadCloud className="mx-auto size-12" /><p className="mt-4 font-display text-4xl">Pusti video za ovaj smer</p></div></div> : null}
  </section>;
}
...
export function TrackExperience({ data, locale, admin = false, inlineLocale = locale, profileName = "student" }: { data: TrackExperienceData; locale: Locale; admin?: boolean; inlineLocale?: Locale; profileName?: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-7">
      <section className="overflow-hidden rounded-[16px] border-2 border-ink bg-white shadow-[8px_8px_0_rgba(14,49,88,0.13)]">
        <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="p-7 sm:p-9">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Link href={`${withLocale(locale, "/app/community/discussions")}?scope=track&track=${data.track.slug}`} className="inline-flex min-h-11 items-center gap-2 rounded-full border-2 border-ink bg-white px-5 text-sm font-black"><MessageCircle className="size-4" /><InlineContentText entityId={trackId} kind="track" field="pageCopy_communityCta" locale={inlineLocale} sr={communityCta?.sr ?? "Zajednica"} en={communityCta?.en ?? "Community"} admin={admin && Boolean(trackId)}>{local(locale, communityCta?.sr ?? "Zajednica", communityCta?.en ?? "Community")}</InlineContentText></Link>
            </div>
          </div>
          <div className="flex min-h-72 flex-col justify-between bg-ink p-7 text-white"><p className="text-xs font-black uppercase tracking-[0.14em] text-white/60">Pregled smera</p><p className="font-display text-5xl">{data.courses.length}</p><p className="text-sm font-black">{data.courses.length === 1 ? "kurs" : "kurseva"}</p></div>
        </div>
      </section>
      ...
      {data.featuredThreads?.length ? <section><h2 className="font-display text-4xl text-ink">Izdvojeno iz zajednice</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{data.featuredThreads.map((thread) => <Link key={thread._id} href={withLocale(locale, `/app/community/${thread._id}`)} className="rounded-[16px] border-2 border-ink bg-white p-5 shadow-[4px_4px_0_rgba(14,49,88,0.1)]"><p className="font-black">{thread.title}</p><p className="mt-2 line-clamp-2 text-sm font-semibold text-muted">{thread.body}</p></Link>)}</div></section> : null}
