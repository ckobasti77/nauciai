...
export default function RichTextEditor({ value, fallback = "", onChange, minHeight = 240 }: { value?: string; fallback?: string; onChange: (json: string, plain: string) => void; minHeight?: number }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "rich-editor-content min-h-[var(--editor-min-height)] px-4 py-4 text-base font-semibold leading-7 text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink",
      },
    },
  });
