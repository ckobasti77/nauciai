...
export function EmailVerificationPage({ locale, token }: { locale: Locale; token?: string }) {
  ...
  const title = isLoading
    ? labelFor(locale, "Potvrđujemo email…", "Confirming your email…")
    : isSuccess
      ? labelFor(locale, "Email je potvrđen!", "Email confirmed!")
      : labelFor(locale, "Link nije moguće iskoristiti", "This link cannot be used");
  const body = isLoading
    ...
    : isSuccess
      ...
      : state === "expired"
        ...
        : state === "used"
          ? labelFor(locale, "Ovaj link je već iskorišćen. Email je već potvrđen ili je zatražen novi link.", "This link has already been used. The email is already confirmed or a newer link was requested.")
          : state === "email_changed"
            ? labelFor(locale, "Email adresa naloga se promenila. Zatraži novi verifikacioni link.", "The account email changed. Request a new verification link.")
            : labelFor(locale, "Link je nevažeći ili je došlo do greške. Zatraži novi link sa stranice Profil.", "The link is invalid or an error occurred. Request a new link from your Profile page.");
      ...
      <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-2xl items-center justify-center">
        <Panel className="w-full p-6 text-center sm:p-10">
          ...
          <p className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-muted">
            ...
            {labelFor(locale, "Verifikacija email-a", "Email verification")}
          </p>
          ...
          {!isSuccess && !isLoading ? (
            <Link href={dashboardHref} className="mt-6 inline-flex text-sm font-black text-ink underline">
              {labelFor(locale, "Otvori Profil i pošalji novi link", "Open Profile and send a new link")}
            </Link>
          ) : null}
        </Panel>
      </div>
