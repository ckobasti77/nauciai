...
function relativeTime(locale: Locale, timestamp?: number, activeNow?: boolean) {
  if (!timestamp) return t(locale, "Aktivnost nije zabeležena", "No activity recorded");
  if (activeNow) return t(locale, "Aktivan sada", "Active now");
  const minutes = Math.max(1, Math.floor((Date.now() - timestamp) / 60_000));
  if (minutes < 60) return t(locale, `pre ${minutes} min`, `${minutes}m ago`);
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return t(locale, `pre ${hours} h`, `${hours}h ago`);
  const days = Math.floor(hours / 24);
  return t(locale, `pre ${days} dana`, `${days}d ago`);
}
    ...
    <div className="fixed inset-0 z-50 grid place-items-end bg-ink/45 p-0 sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
      <div ref={dialogRef} className="max-h-[82vh] w-full overflow-hidden rounded-t-[16px] border-2 border-ink bg-paper shadow-2xl sm:max-w-lg sm:rounded-[16px]">
        <div className="flex items-center justify-between border-b-2 border-line bg-white px-5 py-4">
          ...
          <button type="button" autoFocus onClick={onClose} className="grid size-10 place-items-center rounded-full border-2 border-ink bg-white" aria-label={t(locale, "Zatvori", "Close")}><X className="size-5" /></button>
        </div>
        <div className="max-h-[60vh] space-y-2 overflow-y-auto p-4">
          ...
          {status === "CanLoadMore" ? <button type="button" onClick={() => loadMore(20)} className="w-full rounded-full border-2 border-ink bg-white px-4 py-3 text-sm font-black">{t(locale, "Učitaj još", "Load more")}</button> : null}
        </div>
      </div>
    </div>
  );
...
function ViewerConnectionsList(props: { kind: FollowListKind; username: string; locale: Locale; onClose: () => void }) {
  ...
  return <FollowDialogShell title={props.kind === "followers" ? t(props.locale, "Pratioci", "Followers") : t(props.locale, "Pratim", "Following")} username={props.username} rows={data.results} status={data.status} loadMore={data.loadMore} locale={props.locale} onClose={props.onClose} />;
}
...
function StaffConnectionsList(props: { kind: FollowListKind; userId: Id<"users">; username: string; locale: Locale; onClose: () => void }) {
  ...
  return <FollowDialogShell title={props.kind === "followers" ? t(props.locale, "Pratioci · staff", "Followers · staff") : t(props.locale, "Pratim · staff", "Following · staff")} username={props.username} rows={data.results} status={data.status} loadMore={data.loadMore} locale={props.locale} onClose={props.onClose} />;
}
...
export function MemberProfile({ locale, username }: { locale: Locale; username: string }) {
  ...
  async function follow() {
    ...
    try {
      ...
      setProfileActionNotice(t(locale, "Praćenje trenutno nije izmenjeno. Pokušaj ponovo.", "Following could not be updated. Try again."));
    } finally {
    ...
    try {
      ...
      setProfileActionNotice(
        message.includes("CHAT_BLOCKED")
          ? t(locale, "Razgovor nije dostupan zbog blokiranja.", "This conversation is unavailable because of a block.")
          : message.includes("DM_PRIVACY")
            ? t(locale, "Ova osoba trenutno ne prima nove poruke.", "This person is not accepting new messages right now.")
            : t(locale, "Razgovor trenutno nije moguće otvoriti. Pokušaj ponovo.", "The conversation could not be opened. Try again."),
      );
    } finally {
  ...
  async function inviteToStudy(course: { courseId: Id<"courses">; slug: string }) {
    ...
    try {
      ...
      setStudyNotice(t(locale, "Poziv nije poslat. Proveri dostupnost i zonu napretka u Uči zajedno.", "The invite was not sent. Check availability and progress zone in Study together."));
    } finally {
  ...
  const profileCard = (
    <aside className="rounded-[16px] border-2 border-ink bg-white p-5 shadow-[6px_6px_0_0_rgba(244,190,48,0.75)] xl:sticky xl:top-8">
      ...
      <div className="mt-6 text-center">
        ...
        {profile.progress ? <p className="mt-2 text-xs font-black text-[#2e6f9f]">{profile.progress.xp.toLocaleString()} XP · {t(locale, `Nivo ${profile.progress.level}`, `Level ${profile.progress.level}`)}</p> : null}
        ...
        {profile.help?.status ? <div className="mt-4 rounded-[16px] border-2 border-line bg-paper p-3 text-left"><p className="text-[10px] font-black uppercase text-[#2e6f9f]">{profile.help.status === "seeking" ? t(locale, "Tražim pomoć", "Looking for help") : profile.help.status === "offering" ? t(locale, "Mogu da pomognem", "I can help") : t(locale, "Tražim i nudim pomoć", "Looking for and offering help")}</p>{profile.help.topics.length ? <div className="mt-2 flex flex-wrap gap-1.5">{profile.help.topics.map((topic) => <span key={topic.topicId} className="rounded-full border border-ink bg-white px-2.5 py-1 text-[10px] font-black">{topic.name}</span>)}</div> : null}</div> : null}
      </div>
      ...
      {profile.viewer.isOwner ? <div className="space-y-2"><Link href={`${withLocale(locale, "/app/profile")}#public-profile`} className="flex min-h-12 w-full items-center justify-center rounded-full border-2 border-ink bg-yellow px-4 text-sm font-black shadow-[3px_3px_0_0_#0e3158]">{t(locale, "Uredi javni profil", "Edit public profile")}</Link><Link href={`${withLocale(locale, "/app/profile")}#account-settings`} className="flex min-h-11 w-full items-center justify-center rounded-full border-2 border-ink bg-white px-4 text-sm font-black">{t(locale, "Podešavanja naloga", "Account settings")}</Link>{ownerStatus?.complete === false || ownerStatus?.advisories?.emailVerification || ownerStatus?.advisories?.password ? <div className="rounded-[16px] border-2 border-amber-400 bg-amber-50 p-3 text-left text-xs font-black text-amber-950">{ownerStatus?.complete === false ? <p>{t(locale, "Dovrši korisničko ime i profil.", "Complete your username and profile.")}</p> : null}{ownerStatus?.advisories?.emailVerification ? <p>{t(locale, "Potvrdi email za pristup kursevima.", "Verify email for course access.")}</p> : null}{ownerStatus?.advisories?.password ? <p>{t(locale, "Možeš dodati opcionu lozinku.", "You can add an optional password.")}</p> : null}</div> : null}</div> : (
        ...
        <div className="fixed inset-0 z-[95] grid place-items-end bg-ink/50 p-0 sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="study-invite-title">
          <div ref={studyDialogRef} tabIndex={-1} className="max-h-[82dvh] w-full overflow-hidden rounded-t-[16px] border-2 border-ink bg-white shadow-2xl sm:max-w-lg sm:rounded-[16px]">
            <div className="flex items-start justify-between gap-3 border-b-2 border-ink p-4">
              <div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#2e6f9f]">{t(locale, "Zajednički kurs", "Shared course")}</p><h2 id="study-invite-title" className="mt-1 text-xl font-black">{t(locale, `Uči sa ${identity.name}`, `Study with ${identity.name}`)}</h2></div>
            ...
            <div className="max-h-[62dvh] space-y-2 overflow-y-auto p-4">
              ...
              {commonStudyCourses.results.map((course) => (
                <article key={course.courseId} className="rounded-[16px] border-2 border-line bg-paper p-3">
                  ...
                  {course.matchingAvailable ? <button type="button" disabled={Boolean(studyPendingCourseId)} onClick={() => void inviteToStudy(course)} className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-full border-2 border-ink bg-yellow px-4 text-xs font-black disabled:opacity-50">{studyPendingCourseId === course.courseId ? <Loader2 className="size-4 animate-spin" /> : <GraduationCap className="size-4" />}{t(locale, "Pošalji poziv", "Send invite")}</button> : <button type="button" onClick={() => router.push(`${withLocale(locale, "/app/messages")}?view=study&course=${encodeURIComponent(course.slug)}`)} className="mt-3 inline-flex min-h-10 w-full items-center justify-center rounded-full border-2 border-ink bg-white px-4 text-xs font-black">{t(locale, "Podesi u Uči zajedno", "Set up in Study together")}</button>}
                </article>
              ))}
              {commonStudyCourses.status === "CanLoadMore" || commonStudyCourses.status === "LoadingMore" ? <button type="button" disabled={commonStudyCourses.status === "LoadingMore"} onClick={() => commonStudyCourses.loadMore(12)} className="w-full rounded-full border-2 border-ink bg-white px-4 py-2.5 text-xs font-black disabled:opacity-60">{commonStudyCourses.status === "LoadingMore" ? t(locale, "Učitavanje…", "Loading…") : t(locale, "Učitaj još", "Load more")}</button> : null}
