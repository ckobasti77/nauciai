    ...
    <aside className="rounded-[16px] border-2 border-ink bg-paper-strong p-5 shadow-[6px_6px_0_0_rgba(244,190,48,0.75)] xl:sticky xl:top-8">
      <div className="mx-auto w-fit">
        <div className="relative rounded-full border-[3px] border-ink p-1.5 ring-[7px] ring-yellow/70">
          ...
          {profile.progress ? <span className="absolute -bottom-2 -right-2 grid size-11 place-items-center rounded-full border-[3px] border-paper-strong bg-ink text-lg font-black text-paper-strong">{profile.progress.level}</span> : null}
        </div>
      </div>
      <div className="mt-6 text-center">
        ...
        {profile.help?.status ? <div className="mt-4 rounded-[16px] border-2 border-line bg-paper p-3 text-left"><p className="text-[10px] font-black uppercase text-blue-mid dark:text-muted">{profile.help.status === "seeking" ? t(locale, "Tražim pomoć", "Looking for help") : profile.help.status === "offering" ? t(locale, "Mogu da pomognem", "I can help") : t(locale, "Tražim i nudim pomoć", "Looking for and offering help")}</p>{profile.help.topics.length ? <div className="mt-2 flex flex-wrap gap-1.5">{profile.help.topics.map((topic) => <span key={topic.topicId} className="rounded-full border border-ink bg-paper-strong px-2.5 py-1 text-[10px] font-black">{topic.name}</span>)}</div> : null}</div> : null}
      </div>
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_370px]">
        <div className="order-2 min-w-0 space-y-6 xl:order-1">
          ...
          <section className="rounded-[16px] border-2 border-line bg-paper-strong p-4 sm:p-5">
            ...
            <div className="mt-4 space-y-3">{contributions.status === "LoadingFirstPage" ? <div className="grid py-10 place-items-center"><Loader2 className="size-6 animate-spin" /></div> : contributions.results.length ? contributions.results.map((group) => <article key={group.post.id} className="rounded-[16px] border-2 border-line bg-paper p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase text-blue-mid dark:text-muted">{group.hasThread ? t(locale, "Thread", "Thread") : t(locale, "Komentar", "Comment")}{group.post.course ? ` · ${locale === "sr" ? group.post.course.titleSr : group.post.course.titleEn}` : ""}</p><Link href={withLocale(locale, `/app/community/${group.post.id}`)} className="mt-1 block text-lg font-black text-ink hover:underline">{group.post.title}</Link><p className="mt-2 line-clamp-2 text-sm font-bold leading-6 text-muted">{group.post.body}</p></div><ExternalLink className="size-4 shrink-0 text-muted" /></div>{group.comments.length ? <div className="mt-4 space-y-2 border-t-2 border-line pt-3">{group.comments.map((comment) => <div key={comment.id} className="rounded-[12px] border border-line bg-paper-strong p-3"><p className="text-sm font-bold leading-6 text-ink/85">{comment.body}</p><p className="mt-1 text-[10px] font-black text-muted">{relativeTime(locale, comment.createdAt)}</p></div>)}{group.moreComments ? <p className="text-xs font-black text-blue-mid dark:text-muted">+{group.moreComments} {t(locale, "još", "more")}</p> : null}</div> : null}</article>) : <p className="rounded-[16px] border-2 border-dashed border-line p-8 text-center text-sm font-bold text-muted">{t(locale, "Nema doprinosa za ovaj filter.", "No contributions for this filter.")}</p>}</div>
