    ...
    <div className="fixed inset-0 z-[100] grid place-items-end p-0 sm:place-items-center sm:p-4">
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="new-conversation-title" tabIndex={-1} data-chat-motion-surface="sheet" className="relative flex max-h-[min(760px,92dvh)] w-full flex-col overflow-hidden rounded-t-[16px] border-[3px] border-ink bg-paper-strong shadow-[9px_9px_0_0_var(--shadow-hard-24)] sm:w-[min(560px,100%)] sm:rounded-[16px]">
        ...
        {step === 1 ? <div className="grid gap-3 p-4 sm:grid-cols-2">
          <button type="button" autoFocus onClick={() => { setMode("direct"); setStep(2); }} className="rounded-[16px] border-2 border-ink bg-yellow/20 p-5 text-left transition hover:-translate-y-0.5 hover:bg-yellow/35 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink">
            <MessageCircle className="size-6" /><span className="mt-4 block text-base font-black">{label(locale, "Jedan na jedan", "One to one")}</span><span className="mt-1 block text-xs font-bold leading-5 text-muted">{label(locale, "Izaberi osobu i odmah otvori razgovor.", "Choose a person and open the conversation immediately.")}</span>
          </button>
          <button type="button" onClick={() => { setMode("group"); setStep(2); }} className="rounded-[16px] border-2 border-ink bg-[#d7e9f5] dark:bg-ink/15 p-5 text-left transition hover:-translate-y-0.5 hover:bg-[#c9dfed] dark:hover:bg-ink/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink">
            <Users className="size-6" /><span className="mt-4 block text-base font-black">{label(locale, "Grupa", "Group")}</span><span className="mt-1 block text-xs font-bold leading-5 text-muted">{label(locale, "Dodaj naziv i članove, pa kreiraj grupu.", "Add a name and members, then create the group.")}</span>
          </button>
        </div> : <>
          <div className="border-b border-line p-4">
            <button type="button" onClick={() => setStep(1)} className="mb-3 inline-flex items-center gap-2 rounded-full border border-ink bg-paper-strong px-3 py-1.5 text-[10px] font-black"><ArrowLeft className="size-3.5" />{label(locale, "Promeni vrstu", "Change type")}</button>
            <p className="text-sm font-black">{mode === "group" ? label(locale, "Nova grupa", "New group") : label(locale, "Izaberi osobu", "Choose a person")}</p>
            {mode === "group" ? <label className="mt-3 block text-xs font-black">{label(locale, "Naziv grupe", "Group name")}<input autoFocus value={groupName} onChange={(event) => setGroupName(event.target.value)} maxLength={100} className="mt-1 h-11 w-full rounded-[12px] border-2 border-ink px-3 text-sm font-bold outline-none" placeholder={label(locale, "npr. AI ekipa za učenje", "e.g. AI study group")} /></label> : null}
            ...
            {mode === "group" ? <p className="mt-2 font-mono text-[10px] font-black text-muted">{label(locale, `Izabrano: ${selectedIds.length}`, `Selected: ${selectedIds.length}`)}</p> : null}
...
export function ReportDialog({ locale, target, onClose }: { locale: Locale; target: ReportTarget; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] grid place-items-center p-4">
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="report-dialog-title" tabIndex={-1} data-chat-motion-surface="sheet" className="relative w-[min(440px,100%)] rounded-[16px] border-[3px] border-ink bg-paper-strong p-5 shadow-[8px_8px_0_0_var(--shadow-hard-24)]">
