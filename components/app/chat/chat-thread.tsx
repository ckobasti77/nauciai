...
function MessageBubble({
  ...
  if (message.kind === "system") return <p id={`message-${message.id}`} className={cn("mx-auto my-3 max-w-md rounded-full border border-line bg-paper-strong px-4 py-2 text-center text-[11px] font-black text-muted", highlighted && "ring-4 ring-yellow/70")}>{message.body}</p>;
  ...
  return (
    <article id={`message-${message.id}`} data-chat-motion="bubble" data-chat-motion-new={animateIn ? "true" : undefined} data-chat-motion-direction={mine ? "right" : "left"} className={cn("group flex max-w-[min(88%,42rem)] gap-2 rounded-[16px] transition-shadow", grouped ? "mt-0.5" : "mt-3", mine ? "ml-auto flex-row-reverse" : "mr-auto", pending && "opacity-60", highlighted && "ring-4 ring-yellow/70 ring-offset-2")}>
      ...
      <div className="min-w-0">
        {!mine && !grouped ? <p className="mb-1 px-1 text-[10px] font-black text-muted">{sender?.name}</p> : null}
        <div className={cn("relative rounded-[16px] border-2 border-ink px-3.5 py-2.5 shadow-[2px_2px_0_0_var(--shadow-hard-14)]", mine ? "bg-yellow" : "bg-paper-strong")}>
          ...
          <div className="mt-1.5 flex items-center justify-end gap-1 text-[9px] font-black text-muted">
            ...
            {pending ? <span className="inline-flex items-center gap-1 text-[#2e6f9f] dark:text-muted"><Loader2 className="size-3.5 animate-spin" /><span className="sr-only">{label(locale, "Šalje se", "Sending")}</span></span> : mine ? <span ref={readStatusRef} data-chat-motion="read-status" className="inline-flex items-center gap-1 text-[#2e6f9f] dark:text-muted" title={seenTitle || undefined}>{seenCount ? <><CheckCheck className="size-3.5" /><span>{conversationKind === "group" ? label(locale, `Videlo ${seenCount}`, `Seen by ${seenCount}`) : label(locale, "Viđeno", "Seen")}</span></> : <Check className="size-3.5" />}</span> : null}
          </div>
...
export function ConversationPanel({
  ...
  return (
    ...
    >
      {draggingImages ? <div data-chat-motion="drag-overlay" data-chat-motion-new="true" className={cn("pointer-events-none z-50 grid place-items-center bg-scrim/55 p-5 backdrop-blur-[2px]", compact ? "absolute inset-0" : "fixed inset-0")}><div className="w-[min(92vw,32rem)] rounded-[16px] border-[3px] border-ink bg-yellow p-6 text-center shadow-[8px_8px_0_0_var(--paper-strong)]"><UploadCloud className="mx-auto size-10" /><p className="mt-3 text-xl font-black">{label(locale, "Pusti slike bilo gde u razgovoru", "Drop images anywhere in the conversation")}</p><p className="mt-2 text-xs font-bold">{label(locale, "Do četiri slike, ukupno 25 MB. Sve slike prolaze decode i ponovni encode.", "Up to four images, 25 MB total. Every image is decoded and re-encoded.")}</p></div></div> : null}
      <header className="flex min-h-16 items-center gap-3 border-b-2 border-ink bg-paper-strong px-3 sm:px-4">
      ...
      {messageSearchOpen && !compact ? <div data-chat-motion-surface="panel" className="border-b-2 border-ink bg-[#d7e9f5] dark:bg-ink/15 p-3 sm:p-4">
        <div className="mx-auto max-w-3xl">
          ...
          {messageSearchQuery.trim().length >= 2 ? <div className="mt-2 max-h-56 space-y-1.5 overflow-y-auto rounded-[16px] border-2 border-line bg-paper-strong p-2" aria-live="polite">
            {messageSearch.results.map((result) => <button key={result.messageId} type="button" onClick={() => jumpToMessage(result.messageId)} className="flex w-full items-start gap-3 rounded-[12px] px-3 py-2 text-left hover:bg-paper"><span className="min-w-0 flex-1"><span className="block text-[10px] font-black text-[#2e6f9f] dark:text-muted">{result.senderName}</span><span className="mt-0.5 line-clamp-2 block text-xs font-bold text-ink">{result.body}</span></span><span className="shrink-0 font-mono text-[9px] font-bold text-muted">{searchTimeFormatter.format(new Date(result.createdAt))}</span></button>)}
            ...
            {messageSearch.status === "CanLoadMore" ? <button type="button" onClick={() => messageSearch.loadMore(20)} className="w-full rounded-full border border-ink bg-paper-strong px-3 py-2 text-[10px] font-black">{label(locale, "Učitaj još rezultata", "Load more results")}</button> : null}
      ...
      {(pendingDirect || invited) ? <div data-chat-motion="request" data-chat-motion-new="true" className="border-b-2 border-ink bg-yellow/25 p-3 text-center">
        ...
        {!viewerSentPendingDirect ? <div className="mt-2 flex justify-center gap-2"><button type="button" disabled={Boolean(requestBusy)} onClick={(event) => { requestChatMotion(event.currentTarget, { kind: "reaction" }); void answerPendingRequest(true); }} className="inline-flex min-w-24 items-center justify-center gap-2 rounded-full border-2 border-ink bg-ink px-4 py-2 text-xs font-black text-paper-strong disabled:opacity-50">{requestBusy === "accept" ? <Loader2 className="size-3.5 animate-spin" /> : null}{label(locale, "Prihvati", "Accept")}</button><button type="button" disabled={Boolean(requestBusy)} onClick={(event) => { requestChatMotion(event.currentTarget, { kind: "reaction" }); void answerPendingRequest(false); }} className="inline-flex min-w-24 items-center justify-center gap-2 rounded-full border-2 border-ink bg-paper-strong px-4 py-2 text-xs font-black disabled:opacity-50">{requestBusy === "decline" ? <Loader2 className="size-3.5 animate-spin" /> : null}{label(locale, "Odbij", "Decline")}</button></div> : null}
      </div> : null}

      <div ref={viewportRef} onScroll={handleViewportScroll} role="log" aria-live="polite" aria-relevant="additions text" aria-label={label(locale, "Poruke u razgovoru", "Conversation messages")} data-chat-motion-surface="thread" className="min-h-0 flex-1 overflow-y-auto [overflow-anchor:none] bg-paper p-3 sm:p-4">
        <div ref={listRef}>
          ...
          {targetTooFarBack ? <p role="status" className="mx-auto mt-3 max-w-md rounded-[12px] border-2 border-ink bg-yellow/40 px-4 py-2 text-center text-[11px] font-black">{label(locale, "Poruka je predaleko — otvori punu istoriju.", "Message is too far back — open the full history.")}</p> : null}
          {threadRows.map((row) => row.kind === "day"
            ? <p key={row.key} className="mx-auto mt-3 w-fit rounded-full border border-line bg-paper-strong px-4 py-2 text-center text-[11px] font-black text-muted">{row.text}</p>
            : row.kind === "unread"
              ? <p key={row.key} className="sticky top-0 z-10 mx-auto mt-3 w-fit rounded-full border-2 border-ink bg-yellow px-4 py-1.5 text-center text-[11px] font-black shadow-[2px_2px_0_0_var(--shadow-hard)]">{label(locale, "Nove poruke", "New messages")}</p>
