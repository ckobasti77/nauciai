  ...
  return (
    ...
    >
      ...
      {messageSearchOpen && !compact ? <div data-chat-motion-surface="panel" className="border-b-2 border-ink bg-[#d7e9f5] p-3 sm:p-4">
        <div className="mx-auto max-w-3xl">
          ...
          {messageSearchQuery.trim().length >= 2 ? <div className="mt-2 max-h-56 space-y-1.5 overflow-y-auto rounded-[16px] border-2 border-line bg-white p-2" aria-live="polite">
            {messageSearch.results.map((result) => <button key={result.messageId} type="button" onClick={() => jumpToMessage(result.messageId)} className="flex w-full items-start gap-3 rounded-[12px] px-3 py-2 text-left hover:bg-paper"><span className="min-w-0 flex-1"><span className="block text-[10px] font-black text-[#2e6f9f]">{result.senderName}</span><span className="mt-0.5 line-clamp-2 block text-xs font-bold text-ink">{result.body}</span></span><span className="shrink-0 font-mono text-[9px] font-bold text-muted">{searchTimeFormatter.format(new Date(result.createdAt))}</span></button>)}
      ...
      <div ref={viewportRef} onScroll={handleViewportScroll} role="log" aria-live="polite" aria-relevant="additions text" aria-label={label(locale, "Poruke u razgovoru", "Conversation messages")} data-chat-motion-surface="thread" className="min-h-0 flex-1 overflow-y-auto [overflow-anchor:none] bg-paper p-3 sm:p-4">
        <div ref={listRef}>
          {messages.status === "CanLoadMore" ? <button type="button" onClick={loadOlderMessages} className="mx-auto block rounded-full border-2 border-ink bg-white px-4 py-2 text-xs font-black">{label(locale, "Starije poruke", "Older messages")}</button> : null}
          ...
          {threadRows.map((row) => row.kind === "day"
            ? <p key={row.key} className="mx-auto mt-3 w-fit rounded-full border border-line bg-white px-4 py-2 text-center text-[11px] font-black text-muted">{row.text}</p>
            : row.kind === "unread"
              ? <p key={row.key} className="sticky top-0 z-10 mx-auto mt-3 w-fit rounded-full border-2 border-ink bg-yellow px-4 py-1.5 text-center text-[11px] font-black shadow-[2px_2px_0_0_rgba(14,49,88,0.18)]">{label(locale, "Nove poruke", "New messages")}</p>
        ...
        {!atBottom ? <button type="button" onClick={jumpToLatest} className="sticky bottom-2 mx-auto flex min-h-10 items-center gap-2 rounded-full border-2 border-ink bg-yellow px-4 text-xs font-black shadow-[3px_3px_0_0_rgba(14,49,88,0.18)]"><ChevronDown className="size-4" /><span className="font-mono">{newMessageCount > 0 ? newMessagesLabel(locale, newMessageCount) : label(locale, "Skoči na najnovije", "Jump to latest")}</span></button> : null}
      </div>
