      ...
      {messageSearchOpen && !compact ? <div data-chat-motion-surface="panel" className="border-b-2 border-ink bg-[#d7e9f5] dark:bg-ink/15 p-3 sm:p-4">
        <div className="mx-auto max-w-3xl">
          ...
          {messageSearchQuery.trim().length >= 2 ? <div className="mt-2 max-h-56 space-y-1.5 overflow-y-auto rounded-[16px] border-2 border-line bg-paper-strong p-2" aria-live="polite">
            {messageSearch.results.map((result) => <button key={result.messageId} type="button" onClick={() => jumpToMessage(result.messageId)} className="flex w-full items-start gap-3 rounded-[12px] px-3 py-2 text-left hover:bg-paper"><span className="min-w-0 flex-1"><span className="block text-[10px] font-black text-blue-mid dark:text-muted">{result.senderName}</span><span className="mt-0.5 line-clamp-2 block text-xs font-bold text-ink">{result.body}</span></span><span className="shrink-0 font-mono text-[9px] font-bold text-muted">{searchTimeFormatter.format(new Date(result.createdAt))}</span></button>)}
