    ...
    <ChatMotionScope sceneKey={selected ? String(selected) : view} className="mx-auto flex h-[calc(100dvh-7rem)] min-h-0 max-w-[1500px] min-w-0 flex-col lg:h-[calc(100dvh-4rem)]">
      <header className={cn("mb-4 items-center justify-between gap-4 rounded-[16px] border-2 border-line bg-paper-strong px-4 py-3 shadow-[4px_4px_0_0_var(--shadow-hard-08)] sm:px-5", selected ? "hidden lg:flex" : "flex")}>
        ...
        <div role="tablist" aria-label={label(locale, "Pogled poruka", "Messages view")} className="flex min-w-0 rounded-full border-2 border-ink bg-paper p-1">
          <button type="button" role="tab" aria-selected={view === "conversations"} onClick={() => changeView("conversations")} className={cn("inline-flex min-h-10 items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 text-[10px] font-black sm:gap-2 sm:px-4 sm:text-xs", view === "conversations" ? "bg-ink text-paper-strong" : "text-ink hover:bg-paper-strong")}><MessageCircle className="size-4" />{label(locale, "Razgovori", "Conversations")}{inboxSummary && inboxSummary.totalUnread > 0 ? <span className="rounded-full bg-yellow px-1.5 font-mono text-[9px] text-ink">{inboxSummary.totalUnread > 99 ? "99+" : inboxSummary.totalUnread}</span> : null}</button>
          <button type="button" role="tab" aria-selected={view === "study"} onClick={() => changeView("study")} className={cn("inline-flex min-h-10 items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 text-[10px] font-black sm:gap-2 sm:px-4 sm:text-xs", view === "study" ? "bg-ink text-paper-strong" : "text-ink hover:bg-paper-strong")}><GraduationCap className="size-4" />{label(locale, "Uči zajedno", "Study together")}{studyPendingCount > 0 ? <span className="rounded-full bg-yellow px-1.5 font-mono text-[9px] text-ink">{studyPendingCount > 99 ? "99+" : studyPendingCount}</span> : null}</button>
        </div>
      </header>
