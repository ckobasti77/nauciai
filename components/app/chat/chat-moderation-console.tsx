...
function AppealsPanel({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="rounded-[16px] border-2 border-ink bg-white p-4 sm:p-5">
      <h2 className="text-xl font-black">{t(locale, "Žalbe na suspenziju", "Suspension appeals")}</h2>
      <div className="mt-4 space-y-2">
        ...
        {appeals.status !== "LoadingFirstPage" && appeals.results.length === 0 ? <p className="rounded-[16px] border-2 border-dashed border-line p-5 text-center text-sm font-bold text-muted">{t(locale, "Nema žalbi na čekanju.", "No pending appeals.")}</p> : null}
      </div>
      {selected ? <div className="mt-4 rounded-[16px] border-2 border-ink bg-paper p-4"><label className="text-sm font-black">{t(locale, "Odgovor korisniku", "Response to the member")}<textarea value={response} onChange={(event) => setResponse(event.target.value)} rows={3} className="mt-2 w-full rounded-[8px] border-2 border-ink bg-white p-3 font-bold" /></label><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={pending || response.trim().length < 3} onClick={() => void decide("accepted")} className="inline-flex min-h-10 items-center gap-1 rounded-full border-2 border-ink bg-yellow px-4 text-xs font-black"><Check className="size-4" />{t(locale, "Prihvati žalbu", "Accept appeal")}</button><button type="button" disabled={pending || response.trim().length < 3} onClick={() => void decide("rejected")} className="inline-flex min-h-10 items-center gap-1 rounded-full border-2 border-ink bg-white px-4 text-xs font-black"><X className="size-4" />{t(locale, "Odbij", "Reject")}</button></div></div> : null}
    </section>
  );
}
...
function AdminUserChatAudit({ locale, initialUserId }: { locale: Locale; initialUserId?: string }) {
  ...
  return (
    <section className="rounded-[16px] border-2 border-red-800 bg-red-50 p-4 sm:p-5">
      <p className="text-xs font-black uppercase text-red-800">{t(locale, "Admin pristup sa auditom", "Audited Admin access")}</p>
      <h2 className="mt-1 text-xl font-black">{t(locale, "Pogledaj sve korisnikove chatove", "Inspect a member's chats")}</h2>
      ...
      <div className="mt-4 grid gap-2 lg:grid-cols-[minmax(180px,0.55fr)_minmax(260px,1fr)_auto]">
        ...
        <input value={reason} onChange={(event) => setReason(event.target.value)} maxLength={1_000} placeholder={t(locale, "Obavezan razlog pristupa", "Required access reason")} className="h-11 rounded-[8px] border-2 border-ink bg-white px-3 font-bold" />
...
export function ChatModerationConsole({ locale, role, initialUserId }: { locale: Locale; role: "admin" | "moderator"; initialUserId?: string }) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase text-[#2e6f9f]">{role}</p><h1 className="font-display text-4xl text-ink">{t(locale, "Chat sigurnost", "Chat safety")}</h1><p className="mt-1 text-sm font-bold text-muted">{t(locale, "Prijave, neizmenjivi snapshoti, sankcije i audit pristupa.", "Reports, immutable snapshots, sanctions, and access auditing.")}</p></div><Link href={withLocale(locale, "/app/messages")} className="rounded-full border-2 border-ink bg-white px-4 py-2 text-xs font-black"><MessageSquareText className="mr-2 inline size-4" />{t(locale, "Poruke", "Messages")}</Link></header>

      {role === "admin" ? <AdminUserChatAudit locale={locale} initialUserId={initialUserId} /> : null}

      <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="rounded-[16px] border-2 border-ink bg-white p-4"><div className="flex flex-wrap gap-2">{(["open", "reviewing", "resolved", "dismissed"] as ReportStatus[]).map((value) => <button key={value} type="button" onClick={() => { setStatus(value); setSelectedId(null); }} className={cn("rounded-full border-2 border-ink px-3 py-1.5 text-[10px] font-black uppercase", status === value ? "bg-ink text-white" : "bg-white")}>{reportStatusLabel(locale, value)}</button>)}</div><div className="mt-4 max-h-[700px] space-y-2 overflow-y-auto">{reports.results.map((item) => <button key={item._id} type="button" onClick={() => setSelectedId(item._id)} className={cn("w-full rounded-[16px] border-2 p-3 text-left", selectedId === item._id ? "border-ink bg-yellow/20" : "border-line bg-paper")}><p className="text-[10px] font-black uppercase text-red-700">{item.targetType} · {item.status}</p><p className="mt-1 line-clamp-2 text-sm font-black leading-5">{item.reason}</p><p className="mt-2 text-[10px] font-bold text-muted">{date(locale, item.createdAt)}</p></button>)}{reports.status === "CanLoadMore" ? <button type="button" onClick={() => reports.loadMore(20)} className="w-full rounded-full border-2 border-ink px-3 py-2 text-xs font-black">{t(locale, "Učitaj još", "Load more")}</button> : null}</div></aside>

        <section className="min-w-0 rounded-[16px] border-2 border-ink bg-white p-4 sm:p-5">{!selectedId ? <div className="grid min-h-[420px] place-items-center text-center"><div><ShieldAlert className="mx-auto size-10 text-muted" /><p className="mt-3 font-black text-muted">{t(locale, "Izaberi prijavu.", "Select a report.")}</p></div></div> : report === undefined ? <div className="grid min-h-[420px] place-items-center"><Loader2 className="size-7 animate-spin" /></div> : report === null ? <p>{t(locale, "Prijava nije pronađena.", "Report not found.")}</p> : <div><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase text-red-700">{report.report.targetType} · {report.report.status}</p><h2 className="mt-1 text-2xl font-black">{report.report.reason}</h2><p className="mt-1 text-xs font-bold text-muted">{date(locale, report.report.createdAt)}</p></div><div className="flex gap-2"><button type="button" onClick={() => void setReportStatus("reviewing")} className="rounded-full border-2 border-ink bg-yellow px-3 py-2 text-xs font-black">{t(locale, "Preuzmi", "Review")}</button><button type="button" onClick={() => void setReportStatus("dismissed")} className="rounded-full border-2 border-ink bg-white px-3 py-2 text-xs font-black">{t(locale, "Odbaci", "Dismiss")}</button></div></div><details className="mt-4 rounded-[16px] border-2 border-line bg-paper p-3"><summary className="cursor-pointer text-xs font-black uppercase text-muted">{t(locale, "Neizmenjivi snapshot", "Immutable snapshot")}</summary><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-all text-xs font-bold">{report.report.snapshotJson}</pre></details>{report.report.targetConversationId ? <div className="mt-5"><h3 className="font-black">{t(locale, "Prijavljeni razgovor", "Reported conversation")}</h3><div className="mt-2 max-h-[360px] space-y-2 overflow-y-auto rounded-[16px] border-2 border-line bg-paper p-3">{conversation.results.map((message) => <article key={message._id} className="rounded-[12px] bg-white p-3"><div className="flex justify-between gap-2 text-[10px] font-black text-muted"><span>{message.senderName}</span><span>#{message.sequence}</span></div><p className="mt-1 whitespace-pre-wrap text-sm font-bold leading-6">{message.body || t(locale, "[bez teksta]", "[no text]")}</p></article>)}</div>{conversation.status === "CanLoadMore" ? <button type="button" onClick={() => conversation.loadMore(30)} className="mt-2 rounded-full border-2 border-ink bg-white px-4 py-2 text-xs font-black">{t(locale, "Učitaj još", "Load more")}</button> : null}</div> : null}<div className="mt-5 rounded-[16px] border-2 border-ink bg-paper p-4"><h3 className="flex items-center gap-2 font-black"><Gavel className="size-5" />{t(locale, "Akcija", "Action")}</h3><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} placeholder={t(locale, "Obavezan razlog akcije", "Required action reason")} className="mt-3 w-full rounded-[8px] border-2 border-ink bg-white p-3 font-bold" /><div className="mt-3 flex flex-wrap gap-2">{(["warn", "remove_message", "suspend_chat", "recommend_account_suspension"] as ModerationKind[]).map((value) => <button key={value} type="button" disabled={value === "remove_message" && !report.report.targetMessageId} onClick={() => setKind(value)} className={cn("rounded-full border-2 border-ink px-3 py-2 text-xs font-black disabled:opacity-40", kind === value ? "bg-ink text-white" : "bg-white")}>{moderationKindLabel(locale, value)}</button>)}</div><button type="button" disabled={pending || reason.trim().length < 3} onClick={() => void act()} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-full border-2 border-ink bg-yellow px-5 text-sm font-black shadow-[3px_3px_0_0_#0e3158] disabled:opacity-50"><CircleAlert className="size-4" />{t(locale, "Primeni akciju", "Apply action")}</button>{role === "admin" && report.report.targetUserId ? <div className="mt-4 border-t-2 border-line pt-4"><p className="text-xs font-black uppercase text-red-700">{t(locale, "Puna suspenzija naloga", "Full account suspension")}</p><div className="mt-2 flex flex-wrap gap-2"><select value={duration} onChange={(event) => { setDuration(event.target.value as typeof duration); setSuspendPermanentAck(false); }} className="h-11 rounded-[8px] border-2 border-ink bg-white px-3 text-sm font-black"><option value="24h">24h</option><option value="7d">7d</option><option value="30d">30d</option><option value="permanent">{t(locale, "Trajno", "Permanent")}</option></select><button type="button" disabled={pending || reason.trim().length < 3} onClick={() => setSuspendConfirming(true)} className="rounded-full border-2 border-red-800 bg-red-700 px-4 text-xs font-black text-white disabled:opacity-50">{t(locale, "Suspenduj nalog", "Suspend account")}</button></div>{suspendConfirming ? <div role="alertdialog" aria-label={t(locale, "Potvrda suspenzije naloga", "Confirm account suspension")} className="mt-3 rounded-[16px] border-2 border-red-800 bg-red-50 p-4"><p className="text-sm font-black text-red-900">{t(locale, "Potvrdi suspenziju naloga", "Confirm account suspension")}</p><dl className="mt-2 space-y-1 text-xs font-bold text-red-950"><div><dt className="inline uppercase text-red-700">{t(locale, "Korisnik: ", "User: ")}</dt><dd className="inline font-mono">{report.report.targetUserId}</dd></div><div><dt className="inline uppercase text-red-700">{t(locale, "Trajanje: ", "Duration: ")}</dt><dd className="inline">{suspensionDurationLabel(locale, duration)}</dd></div><div><dt className="inline uppercase text-red-700">{t(locale, "Razlog: ", "Reason: ")}</dt><dd className="inline">{reason}</dd></div></dl>{duration === "permanent" ? <label className="mt-3 flex items-start gap-2 text-xs font-bold text-red-950"><input type="checkbox" checked={suspendPermanentAck} onChange={(event) => setSuspendPermanentAck(event.target.checked)} className="mt-0.5" />{t(locale, "Razumem da je ovo trajna suspenzija bez automatskog isteka.", "I understand this is a permanent suspension with no automatic expiry.")}</label> : null}<div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={pending || reason.trim().length < 3 || (duration === "permanent" && !suspendPermanentAck)} onClick={() => void suspendWholeAccount()} className="inline-flex min-h-10 items-center gap-2 rounded-full border-2 border-red-800 bg-red-700 px-4 text-xs font-black text-white disabled:opacity-50">{pending ? <Loader2 className="size-4 animate-spin" /> : null}{t(locale, "Potvrdi suspenziju", "Confirm suspension")}</button><button type="button" disabled={pending} onClick={() => { setSuspendConfirming(false); setSuspendPermanentAck(false); }} className="rounded-full border-2 border-ink bg-white px-4 py-2 text-xs font-black disabled:opacity-50">{t(locale, "Otkaži", "Cancel")}</button></div></div> : null}</div> : null}{notice ? <p role="status" className={cn("mt-3 text-sm font-black", notice.tone === "error" ? "text-red-800" : "text-emerald-800")}>{notice.text}</p> : null}</div></div>}</section>
      </div>
