...
export function ConversationDetailsDialog({
  ...
  return (
    <div className="fixed inset-0 z-[105] flex justify-end p-0 sm:p-3">
      <button type="button" tabIndex={-1} onClick={onClose} aria-label={label(locale, "Zatvori detalje razgovora", "Close conversation details")} className="absolute inset-0 rounded-none border-0 bg-ink/55 p-0 backdrop-blur-[2px]" style={{ borderRadius: 0 }} />
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="conversation-details-title" tabIndex={-1} data-chat-motion-surface="panel" className="relative flex h-full w-[min(460px,100%)] flex-col overflow-hidden rounded-l-[16px] border-[3px] border-ink bg-white shadow-[-9px_0_24px_rgba(14,49,88,0.2)] sm:rounded-[16px]">
        {draggingAvatar ? <div data-chat-motion="drag-overlay" data-chat-motion-new="true" className="pointer-events-none absolute inset-0 z-50 grid place-items-center bg-ink/80 p-5 text-center text-white"><div className="rounded-[16px] border-2 border-white bg-ink p-6 shadow-[6px_6px_0_0_#f4be30]"><UploadCloud className="mx-auto size-9" /><p className="mt-3 text-lg font-black">{label(locale, "Pusti avatar grupe", "Drop the group avatar")}</p><p className="mt-1 text-xs font-bold text-white/70">{label(locale, "Slika do 5 MB", "Image up to 5 MB")}</p></div></div> : null}
        <div className="flex items-center justify-between gap-3 border-b-2 border-ink p-4"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#2e6f9f]">{isGroup ? label(locale, "Grupa", "Group") : label(locale, "Razgovor", "Conversation")}</p><h2 id="conversation-details-title" className="font-display text-2xl">{label(locale, "Detalji razgovora", "Conversation details")}</h2></div><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-full border-2 border-ink" aria-label={label(locale, "Zatvori", "Close")}><X className="size-4" /></button></div>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
          ...
          <section className="rounded-[16px] border-2 border-line p-3" aria-labelledby="conversation-media-title">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-muted">{label(locale, "Zajednički sadržaj", "Shared content")}</p><h3 id="conversation-media-title" className="text-sm font-black">{label(locale, "Mediji", "Media")}</h3></div><ImagePlus className="size-5 text-[#2e6f9f]" /></div>
            {media.results.length ? <div className="mt-3 grid grid-cols-3 gap-2">{media.results.map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer" className="group relative aspect-square overflow-hidden rounded-[8px] border-2 border-ink bg-paper"><img src={item.url} alt={item.fileName || ""} className="h-full w-full object-cover transition group-hover:scale-[1.03] motion-reduce:transition-none" /><span className="absolute inset-x-0 bottom-0 truncate bg-ink/75 px-1.5 py-1 font-mono text-[8px] font-bold text-white">{new Intl.DateTimeFormat(locale === "sr" ? "sr-Latn" : "en", { day: "2-digit", month: "2-digit" }).format(new Date(item.createdAt))}</span></a>)}</div> : null}
