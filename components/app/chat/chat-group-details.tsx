...
import { ImagePlus, Search, UploadCloud, UserPlus, X } from "lucide-react";
...
export function ConversationDetailsDialog({
  ...
  async function manageMember(kind: "remove" | "transfer", userId: Id<"users">) {
    ...
    try {
      ...
      setNotice(label(locale, "Nije uspelo. Proveri internet, osveži stranicu i pokušaj ponovo.", "That did not work. Check your connection, refresh the page and try again."));
    } finally {
      setBusyKey(undefined);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[105] flex justify-end p-0 sm:p-3">
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="conversation-details-title" tabIndex={-1} data-chat-motion-surface="panel" className="relative flex h-full w-[min(460px,100%)] flex-col overflow-hidden rounded-l-[16px] border-[3px] border-ink bg-paper-strong shadow-[-9px_0_24px_var(--shadow-hard-20)] sm:rounded-[16px]">
        {draggingAvatar ? <div data-chat-motion="drag-overlay" data-chat-motion-new="true" className="pointer-events-none absolute inset-0 z-50 grid place-items-center bg-scrim/80 p-6 text-center text-white"><div className="rounded-[16px] border-2 border-paper-strong bg-ink p-6 shadow-[6px_6px_0_0_var(--yellow)]"><UploadCloud className="mx-auto size-9" /><p className="mt-3 type-h3">{label(locale, "Pusti avatar grupe", "Drop the group avatar")}</p><p className="mt-1 text-xs font-bold text-paper-strong/70">{label(locale, "Slika do 5 MB", "Image up to 5 MB")}</p></div></div> : null}
        <div className="flex items-center justify-between gap-3 border-b-2 border-ink p-4"><div><p className="type-eyebrow-sm text-blue-mid dark:text-muted">{isGroup ? label(locale, "Grupa", "Group") : label(locale, "Razgovor", "Conversation")}</p><h2 id="conversation-details-title" className="font-display type-display-sm">{label(locale, "Detalji razgovora", "Conversation details")}</h2></div><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-full border-2 border-ink" aria-label={label(locale, "Zatvori", "Close")}><X className="size-4" /></button></div>
        <div className="min-h-0 flex-1 space-y-6 overflow-y-auto p-4">
          ...
          <section className="rounded-[16px] border-2 border-line p-3" aria-labelledby="conversation-media-title">
            <div className="flex items-center justify-between gap-3"><div><p className="type-eyebrow-sm text-muted">{label(locale, "Zajednički sadržaj", "Shared content")}</p><h3 id="conversation-media-title" className="type-h4">{label(locale, "Mediji", "Media")}</h3></div><ImagePlus className="size-5 text-blue-mid dark:text-muted" /></div>
            ...
            {media.status === "LoadingFirstPage" ? <div className="grid min-h-24 place-items-center"><Spinner size="md" /></div> : null}
            {!media.results.length && media.status === "Exhausted" ? <p className="mt-3 rounded-[12px] border border-dashed border-line bg-paper p-4 text-center text-xs font-bold text-muted">{label(locale, "Još nema nijedne slike. Zakači sliku uz poruku i pojaviće se ovde.", "No images yet. Attach an image to a message and it will show up here.")}</p> : null}
            {media.status === "CanLoadMore" ? <button type="button" onClick={() => media.loadMore(18)} className="mt-3 w-full rounded-full border border-ink bg-paper-strong px-3 py-2 type-caption font-black">{label(locale, "Učitaj još medija", "Load more media")}</button> : null}
          </section>
          {isGroup ? <>
          {isOwner ? <form onSubmit={rename} className="rounded-[16px] border-2 border-line p-3"><div className="mb-4 flex items-center gap-3"><Avatar src={conversation.conversation.imageUrl} name={name || label(locale, "Grupa", "Group")} size="lg" /><div className="min-w-0 flex-1"><p className="text-xs font-black">{label(locale, "Avatar grupe", "Group avatar")}</p><div className="mt-2 flex flex-wrap gap-2"><label className="cursor-pointer rounded-full border-2 border-ink bg-paper-strong px-3 py-1.5 type-caption font-black"><input type="file" accept="image/*" className="hidden" disabled={busyKey === "avatar"} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAvatar(file); event.currentTarget.value = ""; }} />{busyKey === "avatar" ? label(locale, "Otpremanje…", "Uploading…") : label(locale, "Promeni", "Change")}</label>{conversation.conversation.imageUrl ? <button type="button" disabled={Boolean(busyKey)} onClick={() => void updateGroup({ conversationId, removeImage: true })} className="rounded-full px-3 py-1.5 type-caption font-black text-red-700">{label(locale, "Ukloni", "Remove")}</button> : null}</div></div></div><label className="text-xs font-black">{label(locale, "Naziv grupe", "Group name")}<span className="mt-2 flex gap-2"><input value={name} onChange={(event) => setName(event.target.value)} maxLength={100} className="h-10 min-w-0 flex-1 rounded-[12px] border-2 border-ink px-3 text-sm font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink" /><button type="submit" disabled={busyKey === "rename" || name.trim().length < 2} className="rounded-full border-2 border-ink bg-yellow px-4 text-xs font-black disabled:opacity-40">{label(locale, "Sačuvaj", "Save")}</button></span></label></form> : null}
          {isOwner && !isStudyManaged ? <div className="rounded-[16px] border-2 border-line p-3"><p className="text-xs font-black">{label(locale, "Pozovi člana", "Invite a member")}</p><label className="mt-2 flex h-10 items-center gap-2 rounded-full border-2 border-line bg-paper px-3"><Search className="size-4" /><span className="sr-only">{label(locale, "Pretraži članove", "Search members")}</span><input value={inviteSearch} onChange={(event) => setInviteSearch(event.target.value)} placeholder={label(locale, "Ime ili korisničko ime", "Name or username")} className="min-w-0 flex-1 bg-transparent text-xs font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink" /></label><div className="mt-2 max-h-44 space-y-1.5 overflow-y-auto">{inviteCandidates.map((member) => <button key={member.userId} type="button" onClick={() => void invite(member.userId)} disabled={Boolean(busyKey)} className="flex w-full items-center gap-2 rounded-[12px] border border-line p-2 text-left hover:border-ink disabled:opacity-50"><Avatar src={member.avatarUrl} name={member.name} size="sm" /><span className="min-w-0 flex-1 truncate text-xs font-black">{member.name}</span>{busyKey === `invite:${member.userId}` ? <Spinner size="xs" /> : <UserPlus className="size-3.5" />}</button>)}{candidates.status === "LoadingFirstPage" ? <Spinner size="md" className="mx-auto my-4" /> : null}{candidates.status === "CanLoadMore" ? <button type="button" onClick={() => candidates.loadMore(20)} className="w-full rounded-full border border-ink bg-paper-strong px-3 py-2 type-caption font-black">{label(locale, "Učitaj još", "Load more")}</button> : null}</div></div> : null}
          {isStudyManaged ? <p className="rounded-[12px] border border-line bg-paper p-3 type-caption font-bold text-muted">{label(locale, "Članstvo ove grupe prati Uči zajedno; pozive i uklanjanja menjaj u tom pogledu.", "This membership follows Study together; manage invites and removals in that view.")}</p> : null}
          {invites.results.length || invites.status === "CanLoadMore" ? <div><div className="flex items-center justify-between"><p className="text-xs font-black">{label(locale, "Pozvani", "Invited")}</p><span className="font-mono type-caption font-bold text-muted">{invites.results.length}{invites.status === "CanLoadMore" ? "+" : ""}</span></div><div className="mt-2 space-y-2">{invites.results.map((member) => {
            ...
            return <div key={invitedUserId} data-chat-motion="member" data-chat-motion-new="true" className="flex items-center gap-2 rounded-[12px] border border-dashed border-line bg-paper p-2"><Avatar src={member.avatarUrl} name={member.name ?? label(locale, "Član", "Member")} size="sm" /><span className="min-w-0 flex-1"><span className="block truncate text-xs font-black">{member.name}</span><span className="type-eyebrow-sm text-muted">{label(locale, "Čeka odgovor", "Awaiting response")}</span></span>{isOwner && !isStudyManaged ? <button type="button" onClick={() => void manageMember("remove", invitedUserId)} disabled={Boolean(busyKey)} className={cn("rounded-full border border-red-500 px-2 py-1 type-caption font-black text-red-700", confirmingCancellation && "bg-red-50")}>{confirmingCancellation ? label(locale, "Potvrdi otkazivanje", "Confirm cancellation") : label(locale, "Otkaži poziv", "Cancel invite")}</button> : null}</div>;
          })}</div>{invites.status === "CanLoadMore" ? <button type="button" onClick={() => invites.loadMore(30)} className="mt-2 w-full rounded-full border border-ink px-3 py-2 type-caption font-black">{label(locale, "Učitaj još", "Load more")}</button> : null}</div> : null}
          <div><div className="flex items-center justify-between"><p className="text-xs font-black">{label(locale, "Članovi", "Members")}</p><span className="font-mono type-caption font-bold text-muted">{members.results.length}{members.status === "CanLoadMore" ? "+" : ""}</span></div><div className="mt-2 space-y-2">{members.results.map((member: ConversationMember) => {
            if (!member.userId) return null;
            ...
            return <div key={member.userId} data-chat-motion="member" data-chat-motion-new="true" className="flex items-center gap-2 rounded-[12px] border border-line p-2"><Avatar src={member.avatarUrl} name={member.name ?? label(locale, "Član", "Member")} size="sm" /><span className="min-w-0 flex-1"><span className="block truncate text-xs font-black">{member.name}</span><span className="type-eyebrow-sm text-muted">{member.membershipRole === "owner" ? label(locale, "Vlasnik", "Owner") : label(locale, "Član", "Member")}</span></span>{isOwner && !isViewer ? <span className="flex flex-wrap justify-end gap-1"><button type="button" onClick={() => void manageMember("transfer", member.userId!)} disabled={Boolean(busyKey)} className={cn("rounded-full border border-ink px-2 py-1 type-caption font-black", confirmingTransfer ? "bg-yellow" : "bg-paper-strong")}>{confirmingTransfer ? label(locale, "Potvrdi prenos", "Confirm transfer") : label(locale, "Prenesi", "Transfer")}</button>{!isStudyManaged ? <button type="button" onClick={() => void manageMember("remove", member.userId!)} disabled={Boolean(busyKey)} className={cn("rounded-full border border-red-500 px-2 py-1 type-caption font-black text-red-700", confirmingRemove && "bg-red-50")}>{confirmingRemove ? label(locale, "Potvrdi uklanjanje", "Confirm removal") : label(locale, "Ukloni", "Remove")}</button> : null}</span> : null}</div>;
          })}</div>{members.status === "CanLoadMore" ? <button type="button" onClick={() => members.loadMore(30)} className="mt-2 w-full rounded-full border border-ink px-3 py-2 type-caption font-black">{label(locale, "Učitaj još", "Load more")}</button> : null}</div>
          {conversation.viewer.status === "active" ? <div className="rounded-[16px] border-2 border-red-300 bg-red-50 p-3"><button type="button" onClick={() => void leave()} disabled={isOwner || busyKey === "leave"} className="w-full rounded-full border-2 border-red-600 bg-paper-strong px-4 py-2 text-xs font-black text-red-700 disabled:opacity-45">{busyKey === "leave" ? <Spinner size="xs" className="mr-2 inline" /> : null}{leaveConfirm ? label(locale, "Potvrdi izlazak", "Confirm leaving") : label(locale, "Napusti grupu", "Leave group")}</button>{isOwner ? <p className="mt-2 text-center type-caption font-bold text-red-800">{label(locale, "Vlasnik prvo mora da prenese vlasništvo.", "The owner must transfer ownership first.")}</p> : null}</div> : null}
          </> : null}
        </div>
      </div>
    </div>
