...
export function ChatComposer({
  ...
  return (
    <div className="border-t-2 border-ink bg-paper-strong p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
      {preparedImages.length ? <div className="mb-2 flex gap-2 overflow-x-auto pb-1">{preparedImages.map((image) => <div key={image.imageId} className="relative shrink-0 rounded-[12px] border-2 border-ink bg-paper px-3 py-2 pr-9 type-caption font-black"><ImagePlus className="mb-1 size-4" /><p className="max-w-28 truncate">{image.fileName}</p><p className="font-mono text-muted">{Math.ceil(image.byteSize / 1024)} KB</p><button type="button" onClick={() => onRemoveImage(image.imageId)} className="absolute right-1.5 top-1.5 grid size-6 place-items-center rounded-full border border-ink bg-paper-strong" aria-label={label(locale, "Ukloni sliku", "Remove image")}><X className="size-3" /></button></div>)}</div> : null}
      {imageError ? <p role="alert" className="mb-2 surface-media border border-red-400 bg-red-50 px-3 py-2 type-caption font-black text-red-800">{imageError}</p> : null}
      {sendFailure ? <div role="alert" className="mb-2 flex items-center gap-2 rounded-[8px] border border-red-400 bg-red-50 px-3 py-2"><p className="min-w-0 flex-1 type-caption font-black text-red-800">{sendFailure}</p><button type="button" onClick={onRetry} disabled={sending} className="rounded-full border border-red-700 bg-paper-strong px-3 py-1 type-caption font-black text-red-800 disabled:opacity-50">{label(locale, "Pokušaj ponovo", "Retry")}</button><button type="button" onClick={onDismissFailure} className="grid size-7 place-items-center rounded-full border border-red-300" aria-label={label(locale, "Sakrij grešku", "Dismiss error")}><X className="size-3.5" /></button></div> : null}
      {replyTo ? <div className="mb-2 flex items-center gap-2 surface-media border-l-4 border-ink bg-paper px-3 py-2 type-caption font-bold"><Reply className="size-4" /><span className="min-w-0 flex-1 truncate">{replyTo.sender?.name}: {replyTo.body}</span><button type="button" onClick={onCancelReply} aria-label={label(locale, "Otkaži odgovor", "Cancel reply")}><X className="size-4" /></button></div> : null}
      ...
      <p className="mt-2 px-12 text-center type-caption font-bold text-muted">{disabledReason ?? label(locale, "Poruke nisu end-to-end enkriptovane.", "Messages are not end-to-end encrypted.")}</p>
    </div>
  );
}
