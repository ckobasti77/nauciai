...
export function ChatComposer({
  ...
  return (
    <div className="border-t-2 border-ink bg-white p-3">
      ...
      {sendFailed ? <div role="alert" className="mb-2 flex items-center gap-2 rounded-[8px] border border-red-400 bg-red-50 px-3 py-2"><p className="min-w-0 flex-1 text-xs font-black text-red-800">{label(locale, "Poruka nije poslata.", "Message was not sent.")}</p><button type="button" onClick={onRetry} disabled={sending} className="rounded-full border border-red-700 bg-white px-3 py-1 text-[10px] font-black text-red-800 disabled:opacity-50">{label(locale, "Pokušaj ponovo", "Retry")}</button><button type="button" onClick={onDismissFailure} className="grid size-7 place-items-center rounded-full border border-red-300" aria-label={label(locale, "Sakrij grešku", "Dismiss error")}><X className="size-3.5" /></button></div> : null}
      ...
      <div className="flex items-end gap-2">
        ...
        <label className="min-w-0 flex-1 rounded-[16px] border-2 border-ink bg-paper px-3 py-2 focus-within:bg-white">
          ...
          <textarea value={body} onChange={(event) => { onBodyChange(event.target.value); onTyping(); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onSubmit(); } }} rows={1} placeholder={label(locale, "Napiši poruku…", "Write a message…")} className="max-h-32 min-h-6 w-full resize-none bg-transparent text-sm font-semibold leading-6 outline-none" />
        </label>
        <button type="button" onClick={onSubmit} disabled={sending || (!body.trim() && !preparedImages.length)} className="grid size-11 shrink-0 place-items-center rounded-full border-2 border-ink bg-yellow disabled:opacity-40" aria-label={label(locale, "Pošalji", "Send")}>{sending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</button>
      </div>
