  ...
  return (
    <article className="rounded-[16px] border-2 border-line bg-paper p-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" disabled={pending} onClick={() => void decide("reject")} className="inline-flex min-h-10 items-center gap-1 rounded-full border-2 border-ink bg-paper-strong px-3 text-xs font-black disabled:opacity-50"><X className="size-4" />{t(locale, "Odbij", "Reject")}</button>
      </div>
      {sameScopeTopics.length ? <div className="mt-3 flex flex-wrap gap-2 border-t border-line pt-3"><select value={mergeTopicId} onChange={(event) => setMergeTopicId(event.target.value)} className="h-10 min-w-0 flex-1 rounded-[8px] border-2 border-ink bg-paper-strong px-2 text-xs font-black"><option value="">{t(locale, "Spoji sa postojećom…", "Merge into existing…")}</option>{sameScopeTopics.map((topic) => <option key={topic.topicId} value={topic.topicId}>{topic.name}</option>)}</select><button type="button" disabled={pending || !mergeTopicId} onClick={() => void decide("merge")} className="rounded-full border-2 border-ink bg-paper-strong px-4 text-xs font-black disabled:opacity-50">{t(locale, "Spoji", "Merge")}</button></div> : null}
    </article>
  );
}
...
function HelpTopicModeration({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="rounded-[16px] border-2 border-ink bg-paper-strong p-4 shadow-[4px_4px_0_0_var(--shadow-hard-10)] sm:p-6">
      <p className="type-eyebrow text-blue-mid dark:text-muted">Admin</p>
      <h3 className="mt-2 type-h3 text-ink">{t(locale, "Predlozi tema na čekanju", "Pending topic suggestions")}</h3>
      <div className="mt-4 space-y-2">
        {suggestions.results.map((suggestion) => <PendingTopicRow key={suggestion._id} locale={locale} suggestion={suggestion} />)}
        {suggestions.status !== "LoadingFirstPage" && suggestions.results.length === 0 ? <p className="rounded-[16px] border-2 border-dashed border-line p-6 text-center text-sm font-bold text-muted">{t(locale, "Nema predloga na čekanju.", "No pending suggestions.")}</p> : null}
        {suggestions.status === "CanLoadMore" ? <button type="button" onClick={() => suggestions.loadMore(10)} className="w-full rounded-full border-2 border-ink bg-paper-strong px-4 py-2 text-xs font-black">{t(locale, "Učitaj još", "Load more")}</button> : null}
      </div>
    </section>
  );
}
...
export function HelpSettings({ locale, role }: { locale: Locale; role: string }) {
  ...
  return (
    <div id="community-help" className="mt-6 scroll-mt-6 space-y-6">
      <section className="rounded-[16px] border-2 border-ink bg-paper-strong p-4 shadow-[5px_5px_0_0_rgba(244,190,48,0.7)] sm:p-6">
        <div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-full border-2 border-ink bg-yellow"><HandHeart className="size-5" /></span><div><h2 className="type-h2 text-ink">{t(locale, "Pomoć zajednice", "Community help")}</h2><p className="mt-1 type-body-sm font-bold text-muted">{t(locale, "Izaberi status i do pet odobrenih tema. Partnerstva za učenje sada su u Porukama.", "Choose a status and up to five approved topics. Study partnerships now live in Messages.")}</p></div></div>
        <div className="mt-5 flex flex-wrap gap-2" role="group" aria-label={t(locale, "Status pomoći", "Help status")}>{(["seeking", "offering", "both"] as HelpMode[]).map((mode) => <button key={mode} type="button" onClick={() => setDraft({ status: mode, topics: current.topics })} className={cn("rounded-full border-2 border-ink px-4 py-2 text-xs font-black", current.status === mode ? "bg-ink text-paper-strong" : "bg-paper-strong text-ink")}>{modeLabel(locale, mode)}</button>)}<button type="button" onClick={() => setDraft({ status: null, topics: [] })} className={cn("rounded-full border-2 border-ink px-4 py-2 text-xs font-black", current.status === null ? "bg-ink text-paper-strong" : "bg-paper-strong text-ink")}>{t(locale, "Isključeno", "Off")}</button></div>
        <label className="mt-5 block text-sm font-black text-ink">{t(locale, "Prikaži teme", "Show topics")}<select value={scopeCourseId} onChange={(event) => setScopeCourseId(event.target.value)} className="mt-2 h-11 w-full rounded-[8px] border-2 border-ink bg-paper-strong px-3 font-bold sm:max-w-md"><option value="">{t(locale, "Globalne teme", "Global topics")}</option>{(courses ?? []).map((course) => <option key={course._id} value={course._id}>{locale === "sr" ? course.titleSr : course.titleEn}</option>)}</select></label>
        <div className="mt-4 grid gap-2 md:grid-cols-2">{visibleTopics.map((topic) => { const checked = selected(topic.topicId); const selectedTopic = current.topics.find((item) => item.topicId === topic.topicId); return <div key={topic.topicId} className={cn("rounded-[16px] border-2 p-3", checked ? "border-ink bg-yellow/20" : "border-line bg-paper")}><button type="button" onClick={() => toggleTopic(topic)} className="flex w-full items-center gap-3 text-left"><span className={cn("grid size-6 place-items-center rounded-full border-2 border-ink", checked ? "bg-yellow" : "bg-paper-strong")}>{checked ? <Check className="size-4" /> : null}</span><span className="min-w-0 flex-1 truncate font-black text-ink">{topic.name}</span></button>{checked && selectedTopic ? <select value={selectedTopic.mode} onChange={(event) => setDraft({ ...current, topics: current.topics.map((item) => item.topicId === topic.topicId ? { ...item, mode: event.target.value as HelpMode } : item) })} className="mt-3 h-9 w-full rounded-[8px] border-2 border-ink bg-paper-strong px-2 text-xs font-black"><option value="seeking">{modeLabel(locale, "seeking")}</option><option value="offering">{modeLabel(locale, "offering")}</option><option value="both">{modeLabel(locale, "both")}</option></select> : null}</div>; })}{activeTopics !== undefined && visibleTopics.length === 0 ? <p className="rounded-[16px] border-2 border-dashed border-line p-6 text-sm font-bold text-muted">{t(locale, "Još nema odobrenih tema u ovom opsegu.", "There are no approved topics in this scope yet.")}</p> : null}</div>
        <div className="mt-5 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input value={proposal} onChange={(event) => setProposal(event.target.value)} maxLength={80} placeholder={t(locale, "Predloži novu temu…", "Suggest a new topic…")} className="h-11 rounded-[8px] border-2 border-ink bg-paper-strong px-3 font-bold" /><button type="button" disabled={proposing || !proposal.trim()} onClick={() => void propose()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border-2 border-ink bg-paper-strong px-4 text-xs font-black disabled:opacity-50">{proposing ? <Spinner /> : <Plus className="size-4" />}{t(locale, "Pošalji predlog", "Send suggestion")}</button></div>
        <div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" disabled={saving || profile === undefined} onClick={() => void save()} className="inline-flex min-h-11 items-center gap-2 rounded-full border-2 border-ink bg-yellow px-5 text-sm font-black shadow-[3px_3px_0_0_var(--ink)] disabled:opacity-50">{saving ? <Spinner /> : <Check className="size-4" />}{t(locale, "Sačuvaj status", "Save status")}</button>{notice ? <p role="status" className="text-sm font-black text-ink">{notice}</p> : null}</div>
      </section>
