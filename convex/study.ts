  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.db
      ...
      .withIndex("by_courseId_and_progressZone_and_active_and_updatedAt", (q) =>
        q.eq("courseId", args.courseId).eq("progressZone", progress.progressZone).eq("active", true),
      )
      .order("desc")
    ...
    for (const row of result.page) {
      ...
      const liveProgress = await courseProgress(ctx, row.userId, args.courseId);
      if (liveProgress.progressZone !== progress.progressZone) continue;
      const member = await memberSummary(ctx, row.userId);
      if (member) page.push({ ...member, progressPercent: liveProgress.progressPercent, progressZone: liveProgress.progressZone });
    }
    return { ...result, page, viewerProgress: progress };
  },
});
...
export const createPartnerInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await schedulePersistentActivityPush(ctx, {
      ...
      urlPath: "/app/profile#social-learning",
...
export const cancelPartnerInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(invite._id, { status: "cancelled", updatedAt: Date.now() });
    return null;
  },
});
...
export const respondToPartnerInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const partnershipId = existingPartnership
      ? (await ctx.db.patch(existingPartnership._id, { directConversationId: conversationId, updatedAt: now }), existingPartnership._id)
    ...
    await ctx.db.patch(invite._id, { status: "accepted", cooldownUntil: undefined, respondedAt: now, updatedAt: now });
...
export const respondToStudyGroupInvite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    if (args.decision === "decline") {
      await ctx.db.patch(invite._id, { status: "declined", respondedAt: now });
    ...
    if (existingMember) {
      await ctx.db.patch(existingMember._id, { active: true, leftAt: undefined, joinedAt: now });
    } else {
      await ctx.db.insert("studyGroupMembers", {
        groupId: group._id,
        userId: actor.userId,
...
export const leaveStudyGroup = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    await ctx.db.patch(membership._id, { active: false, leftAt: now });
    if (group.conversationId) {
      const chatMembership = await ctx.db
        ...
        .unique();
      if (chatMembership) await ctx.db.patch(chatMembership._id, { status: "left", leftAt: now, updatedAt: now });
    }
